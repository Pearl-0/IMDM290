...
public class Lerp0 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < numSphere; i++)
        {
            ...
            spheres[i] = GameObject.CreatePrimitive(PrimitiveType.Capsule);
            ...
            //Color color = Color.HSVToRGB(0.9f, hue, .5f); // Full saturation and brightness
           Color color = Color.HSVToRGB(0.5f, hue, .6f);
            //Color color = Color.HSVToRGB(Random.Range(0.8f, 1f), Random.Range(0.8f, 0.94f), Random.Range(3f, 5f));
    ...
    void Update()
    {
        ...
        for (int i = 0; i < numSphere; i++)
        {
            ...
            lerpFraction = Mathf.Sin(time * 0.5f) * 0.5f + 0.5f;
            ...
            Renderer sphereRenderer = spheres[i].GetComponent<Renderer>();
            float hue = (float)i *2/ numSphere; // Hue cycles through 0 to 1
            Color color = Color.HSVToRGB(Mathf.Abs(hue * Mathf.Sin(time * 0.5f)), Mathf.Cos(time * 0.5f), 1.0f); // Full saturation and brightness
            sphereRenderer.material.color = color;
        }
